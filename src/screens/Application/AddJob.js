...
const handleEducationSelect = (education) => {
  ...
  const fetchEmpAndCompId = async() => {
    ...
    try {
      ...
      const CompResponse = await axios.get(API_ENDPOINTS.COMPANY_DETAILS, {params: {user_id: +userId}})
  ...
  const handlePost = async() => {
    ...
    try {
      const payload =  job ? { 
        ...
        receive_applicants_by: receiveApplicantsBy,         // string
        email: email,                                       // string
    ...
    return (
      <SafeAreaView style={styles.container}>
          ...
          <AppBar
            ...
            leading={
              <View style={{ padding: 10, gap: 10 }}>
                ...
                <Text style={{ fontFamily: "Poppins-SemiBold", fontSize: 20, marginLeft: 10 }}>Describe Your Jobs</Text>
              </View>
            }
        ...
        <View style={styles.tabContainer}>
          ...
          >
            <Text style={[styles.tabText, activeTab === "job-details" && styles.activeTabText]}>Job Details</Text>
          </TouchableOpacity>
          <TouchableOpacity
     ...
     {job &&   <View style={styles.switchContainer}>
        <Text style={{ fontFamily: "Poppins-Medium", fontSize: 14 }}>Job Status: </Text>
        <Switch value={job_status === "Active"} onValueChange={(value) => setJobStatus(value ? "Active" : "Inactive")} />
        <Text style={{ fontFamily: "Poppins-Medium", fontSize: 14 , backgroundColor: job_status === "Active" ? "#E4FFFD" : "#FFEFEF" , padding: 5 , borderRadius: 5 , color: job_status === "Active" ? Colors.primary : "#ff5c5c" }}>{job_status}</Text>
       </View>}
            ...
            <View style={styles.formContainer}>
              ...
              <View style={styles.fieldContainer}>
                ...
                {showLocationInput ? (
                  ...
                  <TouchableOpacity style={styles.addLocationButton} onPress={() => setShowLocationInput(true)}>
                    <Ionicons name="add" size={18} color="#50B5A3" />
      ...
      return (
        <TouchableOpacity
          ...
          style={[
            ...
            isSelected && styles.selectedEducationBadge
          ]}
        ...
        >
          ...
          {isSelected && (
            <View style={styles.checkmarkContainer}>
              ...
              <View style={[styles.fieldContainer, {marginTop: 20, marginBottom: 10}]}>
                <Text style={styles.sectionTitle}>How would you like to receive your applicants?</Text>
              </View>

              {/* Receive Applicants By */}
              <View style={[styles.fieldContainer]}>
                <Text style={styles.label}>
                  Receive Applicants By: <Text style={styles.required}>*</Text>
                </Text>
                <DropDownPicker
                  listMode="SCROLLVIEW"
                  scrollViewProps={{ nestedScrollEnabled: true }}
                  open={receiveByOpen}
                  setOpen={(open) => {
                    if (open) handleOpenDropdown('receive')
                    else setReceiveByOpen(false)
                  }}
                  value={receiveApplicantsBy}
                  setValue={setReceiveApplicantsBy}
                  items={[
          ...
          {activeTab === "requirements" && (
            <View style={styles.formContainer}>
              ...
              <TouchableOpacity style={styles.addRequirementButton} onPress={() => setShowAttachment(!showAttachment)}>
