...
import { setFullName } from "../../redux/slice/introSlice"
...
export default function Home() {
  ...
  const fetchUserIdAndDetail = async () => {
    try {
      ...
      if (storedUserId) {
        ...
        dispatch(setFullName(response.data.data.full_name))
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Cleanup interval after 6 seconds or when component unmounts
